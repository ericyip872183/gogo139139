...
> 最后更新：2026-03-26
> 状态：待开发
> 使用模型：火山引擎豆包（doubao-seed-2.0-lite，支持切换）
...
| 特性 | 说明 |
|------|------|
| **重复检测** | 导入前自动检测题库中是否已有相同或高度相似的题目，避免重复录入 |
| **答案校验** | 前端自动校验答案格式是否正确（如多选题答案必须是字母组合） |
| **模型可选** | 支持切换不同的豆包大模型，平衡识别速度和准确率 |
...
6. **重复检测**：导入前自动检测重复题目，避免重复录入
7. **模型可选**：支持切换不同的豆包大模型（默认 doubao-seed-2.0-lite）
8. **答案校验**：前端自动校验答案格式，提示用户修正
...
┌─────────────────────────────────────────────────────────────────┐
│  AI 大模型智能解析（火山引擎豆包）                                 │
│  - 模型可选：doubao-seed-2.0-lite / doubao-pro-32k / ...       │
...
function similarityScore(s1: string, s2: string): number {
  const jaccard = jaccardSimilarity(extractKeywords(s1), extractKeywords(s2))
  const maxLen = Math.max(s1.length, s2.length)
  const editSim = 1 - editDistance(s1, s2) / maxLen
  return jaccard * 0.6 + editSim * 0.4  // 加权平均
}
```
...
| 检测结果 | 处理方式 |
|---------|---------|
| 完全重复（MD5 匹配） | 默认跳过，提示"该题目已存在于题库中" |
| 高度相似（≥85%） | 高亮显示相似题目，用户选择跳过/覆盖/保留两者 |
| 可能相似（60-85%） | 弱提示"题库中可能存在相似题目"，用户确认后导入 |

### 3.2 答案格式自动校验规则

前端在用户确认导入前，自动校验答案格式是否正确：

| 题型 | 答案格式要求 | 校验规则 | 错误示例 |
|------|-------------|---------|---------|
| **单选题** | 单个字母（A/B/C/D） | `/^[A-D]$/` | `AB`、`1`、` 甲` |
| **多选题** | 大写字母组合（2-5 个） | `/^[A-E]{2,5}$/` | `A`、`ABCD1`、`a,b,c` |
| **判断题** | A 或 B（正确/错误） | `/^[AB]$/` | `正确 `、`true`、`√` |
| **填空题** | 非空字符串 | `answer.trim().length > 0` | 空字符串、仅空格 |

**校验失败处理**：
- 答案框显示红色边框
- 下方显示错误原因（如"单选题答案必须是 A/B/C/D 中的一个"）
- 该题目无法被选中导入
- 提供"快速修正"按钮（如自动提取括号内的答案）

### 3.3 模型选择策略

| 模型 | 适用场景 | 速度 | 成本 | 推荐 |
|------|---------|------|------|------|
| **doubao-seed-2.0-lite** | 标准试卷、清晰文本 | 快 (~5s/25 题) | ¥0.0003/1K tokens | ✅ 默认推荐 |
| **doubao-pro-32k** | 复杂试卷、扫描版 PDF | 中 (~15s/25 题) | ¥0.0008/1K tokens | 高精度场景 |
| **doubao-pro-4k** | 单页题目、少量题目 | 快 (~3s/10 题) | ¥0.0008/1K tokens | 小批量导入 |
...
│                                                               │
│  大模型选择：[doubao-seed-2.0-lite ▼]  (?)                   │
│  💡 doubao-seed-2.0-lite：识别速度快，适合标准试卷（推荐）    │
│     doubao-pro-32k：识别精度高，适合扫描版/复杂试卷          │
│                                                               │
│  ┌─────────────────────────────────────────────────────────┐ │
│  │                                                         │ │
│  │           📁 点击或拖拽文件到此处                        │ │
...
│  └────────────────────────────────────────────────────────────┘  │
│                                                                  │
│  ☑ #3 [单选题] ⚠️ 发现相似题目（相似度 87%）                      │
│  ┌────────────────────────────────────────────────────────────┐  │
│  │ 题目：下列哪项不属于中医诊断的基本原则？                   │  │
│  │                                                            │  │
│  │ 相似题目：中医诊断学的基本原理不包括：（题库中已存在）      │  │
│  │                                                            │  │
│  │ 处理建议：○ 跳过此题   ● 保留两者   ○ 覆盖原有            │  │
│  └────────────────────────────────────────────────────────────┘  │
│                                                                  │
...
### Phase 1: 数据库与服务层（3 天） ✅ 已完成
- [x] 数据库表：`AiImportTask`, `AiImportItem`
- [x] `FileUploadService`: 文件上传/转 base64
- [x] `AiParserService`: AI 大模型解析（豆包文件上传）
- [x] `AiImportService`: 导入任务管理
- [x] 火山引擎 API 配置（豆包大模型）

### Phase 2: 接口层（2 天） ✅ 已完成
- [x] `AiImportController`: 上传/查询/确认接口
- [x] 文件上传中间件（multer 配置）
- [x] 异步任务队列（简单实现，无需 Bull）
- [x] OSS 文件存储（本地临时存储，24 小时清理）

### Phase 3: 前端界面（3 天） ✅ 已完成
- [x] `AiImportDialog.vue`: 上传弹窗（ QuestionsView.vue 内嵌）
- [x] `AiImportProgress.vue`: 进度展示（内嵌）
- [x] `AiImportReview.vue`: 校对界面（内嵌）
- [x] `AiImportEdit.vue`: 题目编辑（内嵌）
- [x] `AiImportHistory.vue`: 历史记录（后续实现）
...
以火山引擎豆包大模型为例（价格来源：火山引擎官网）：

| 模型 | 单价 | 25 题试卷成本 | 100 题试卷成本 |
|------|------|-------------|--------------|
| **doubao-seed-2.0-lite** | ¥0.0003/1K tokens | ¥0.015 | ¥0.06 |
| **doubao-pro-4k** | ¥0.0008/1K tokens | ¥0.04 | ¥0.16 |
| **doubao-pro-32k** | ¥0.0008/1K tokens | ¥0.04 | ¥0.16 |

**估算假设**：
- 每道题目（含题干 + 选项 + 答案 + 解析）约 200-300 tokens
- 25 题试卷约 5,000-7,500 tokens
- 100 题试卷约 20,000-30,000 tokens

### 10.2 月度成本预估

| 场景 | 月导入量 | 推荐模型 | 预估成本 |
|------|---------|---------|---------|
| **小型机构** | 50 份试卷（25 题/份） | doubao-seed-2.0-lite | ¥0.75/月 |
| **中型机构** | 200 份试卷（25 题/份） | doubao-seed-2.0-lite | ¥3.00/月 |
| **大型机构** | 1000 份试卷（25 题/份） | doubao-seed-2.0-lite | ¥15.00/月 |
| **高精度场景** | 100 份扫描版试卷 | doubao-pro-32k | ¥4.00/月 |

### 10.3 成本优化建议

1. **默认使用 lite 模型**：90% 场景使用 doubao-seed-2.0-lite
2. **按需切换 pro 模型**：仅扫描版/复杂试卷使用 pro 模型
3. **批量识别**：多文件合并识别，减少重复 prompt 开销
4. **缓存机制**：相同文件 MD5 命中时直接返回缓存结果

---

## 十一、后续优化方向

1. **题目去重检测**：导入前检查题库中是否已有相同题目 ✅ 已实现
2. **智能分类推荐**：AI 根据题目内容推荐分类
3. **难度自动识别**：AI 根据题目内容自动判断难度
4. **模板学习**：用户上传多次后，AI 学习该用户的题目格式偏好
5. **批量导出校对结果**：支持导出 AI 识别结果为 Word/Excel，便于线下核对
6. **答案格式校验**：前端自动校验答案格式 ✅ 已实现
7. **模型选择**：支持切换不同豆包模型 ✅ 已实现
