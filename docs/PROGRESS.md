...
| 批量删除/移动分类 | ✅ | |
| 题目内容支持富文本（TinyMCE） | ✅ | 支持格式化文本、图片、表格等 |
| 题目媒体资源管理 | ✅ | QuestionMedia 表，支持图片/视频/音频上传 |
| 前端：题库管理页（左树右列表） | ✅ | QuestionsView.vue（富文本 + 媒体上传） |
...
| 6级角色权限体系（SUPER_ADMIN/TENANT_ADMIN/CLASS_ADMIN/TEACHER/STUDENT） | ✅ | 替换原有3级体系；高级别可见低级别数据；同级隔离；角色分配只能向下；TENANT_ADMIN每机构唯一 | 2026-03-19 |
...
| 机构管理员/班级管理员导航菜单 | ✅ | 在 MainLayout 中为机构管理员和班级管理员显示「组织架构」和「用户管理」菜单 | 2026-03-24 |
| AI 大模型管理 - 后端 | ✅ | 超管端点：服务商/模型/机构模型/配额/预警/统计 CRUD；机构端点：服务查看/充值/配额申请 | 2026-03-24 |
| AI 大模型管理 - 前端 | ✅ | 超管：AI 平台管理页（服务商/模型/机构模型/配额/统计）；机构：AI 服务中心（服务/统计/充值/预警） | 2026-03-24 |
| AI 流式对话（SSE） | ✅ | 支持 Server-Sent Events 流式输出，文字逐字显示，带打字动画 | 2026-03-25 |
| AI 图片生成功能 | ✅ | 支持文生图配置（尺寸/质量/风格/数量），网格展示结果，支持下载 | 2026-03-25 |
| 题库富文本编辑器 + 媒体资源 | ✅ | 题目内容支持富文本（TinyMCE），支持图片/视频/音频上传和管理，QuestionMedia 表 | 2026-03-25 |
| AI 智能导入 PDF 文档 | ✅ | 使用火山引擎 Responses API 文档理解端点，支持 PDF/Word 文档解析，AI 自动提取题目并结构化（单选/多选/判断/填空），重复检测，待校对入库 | 2026-03-29 |
