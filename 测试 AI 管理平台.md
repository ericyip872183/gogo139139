...
|------|------|------|
| 名称 | 必填 | 服务商名称，如火山引擎豆包 |
| API 地址 | 必填 | 完整的 URL，如 https://ark.cn-beijing.volces.com/api/v3 |
| 认证方式 | 必填 | 默认选择 Bearer Token |
| API Key | 必填 | 从服务商控制台获取的 API Key |
| API Secret | 可选 | HMAC 认证方式需要，Bearer Token 不需要填写 |
| 状态 | 必填 | 默认已启用 |
